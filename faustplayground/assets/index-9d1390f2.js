...
`;
    ...
    FaustModule = (await __vitePreload(() => import(
      /* webpackIgnore: true */
      jsFileMod
    ), true ? [] : void 0)).default;
    ...
    const { promises: fs } = await __vitePreload(() => import("./__vite-browser-external-d06ac358.js"), true ? [] : void 0);
    const { pathToFileURL } = await __vitePreload(() => import("./__vite-browser-external-d06ac358.js"), true ? [] : void 0);
...
var _FaustCompiler = class _FaustCompiler2 {
  /**
  ...
  async createDSPFactory(name, code, args, poly) {
    if (_FaustCompiler2.gFactories.size > 10) {
      _FaustCompiler2.gFactories.clear();
    }
    const shaKey = await sha256(
      name + code + args + (poly ? "poly" : "mono")
    );
    if (_FaustCompiler2.gFactories.has(shaKey)) {
      return _FaustCompiler2.gFactories.get(shaKey) || null;
    } else {
      try {
        ...
        _FaustCompiler2.gFactories.set(shaKey, factory);
        return factory;
      } catch (e) {
...
var SoundfileReader = class {
  ...
  static async loadSoundfile(filename, metaUrls, soundfiles, audioCtx) {
    if (soundfiles == null ? void 0 : soundfiles[filename])
      return;
    const urlsToCheck = [
      filename,
...
var _FaustMonoDspGenerator = class _FaustMonoDspGenerator2 {
  constructor() {
    this.factory = null;
  }
  ...
  async createNode(context, name = this.name, factory = this.factory, sp = false, bufferSize = 1024, processorName = (factory == null ? void 0 : factory.shaKey) || name, processorOptions = {}) {
    ...
    if (sp) {
      ...
      if (!_FaustMonoDspGenerator2.gWorkletProcessors.has(context))
        _FaustMonoDspGenerator2.gWorkletProcessors.set(
          context,
          /* @__PURE__ */ new Set()
        );
      if (!((_a = _FaustMonoDspGenerator2.gWorkletProcessors.get(context)) == null ? void 0 : _a.has(processorName))) {
        try {
          const processorCode = `
...
`;
          ...
          (_b = _FaustMonoDspGenerator2.gWorkletProcessors.get(context)) == null ? void 0 : _b.add(processorName);
        } catch (e) {
          throw e;
        }
      }
  ...
  async createFFTNode(context, fftUtils, name = this.name, factory = this.factory, fftOptions = {}, processorName = (factory == null ? void 0 : factory.shaKey) ? `${factory.shaKey}_fft` : name, processorOptions = {}) {
    ...
    if (!_FaustMonoDspGenerator2.gWorkletProcessors.has(context))
      _FaustMonoDspGenerator2.gWorkletProcessors.set(context, /* @__PURE__ */ new Set());
    if (!((_a = _FaustMonoDspGenerator2.gWorkletProcessors.get(context)) == null ? void 0 : _a.has(processorName))) {
      try {
        const processorCode = `
// DSP name and JSON string for DSP are generated
...
var _FaustPolyDspGenerator = class _FaustPolyDspGenerator2 {
  constructor() {
...
`;
  ...
  async createNode(context, voices, name = this.name, voiceFactory = this.voiceFactory, mixerModule = this.mixerModule, effectFactory = this.effectFactory, sp = false, bufferSize = 1024, processorName = ((voiceFactory == null ? void 0 : voiceFactory.shaKey) || "") + ((effectFactory == null ? void 0 : effectFactory.shaKey) || "") || `${name}_poly`, processorOptions = {}) {
    ...
    if (sp) {
      ...
      if (!_FaustPolyDspGenerator2.gWorkletProcessors.has(context))
        _FaustPolyDspGenerator2.gWorkletProcessors.set(
          context,
          /* @__PURE__ */ new Set()
        );
      if (!((_a = _FaustPolyDspGenerator2.gWorkletProcessors.get(context)) == null ? void 0 : _a.has(processorName))) {
        try {
          const processorCode = `
// DSP name and JSON string for DSP are generated
const faustData = ${JSON.stringify({
...
`;
          ...
          await context.audioWorklet.addModule(url);
          (_b = _FaustPolyDspGenerator2.gWorkletProcessors.get(context)) == null ? void 0 : _b.add(processorName);
        } catch (e) {
          throw e;
        }
      }
  ...
  async createAudioWorkletProcessor(name = this.name, voiceFactory = this.voiceFactory, effectFactory = this.effectFactory, processorName = ((voiceFactory == null ? void 0 : voiceFactory.shaKey) || "") + ((effectFactory == null ? void 0 : effectFactory.shaKey) || "") || `${name}_poly`) {
    ...
    const voiceMeta = JSON.parse(voiceFactory.json);
    const effectMeta = effectFactory ? JSON.parse(effectFactory.json) : void 0;
    voiceMeta.compile_options.match("-double") ? 8 : 4;
    try {
      const dependencies = {
        FaustBaseWebAudioDsp,
...
var _FaustDspGenerator = class _FaustDspGenerator2 {
  ...
  async createFaustNode(context, name, code, sp, bufferSize) {
    const getCompiler = async () => {
      if (!_FaustDspGenerator2.compilerPromise) {
        ...
        _FaustDspGenerator2.compilerPromise = instantiateFaustModuleFromFile_default(
          jsURL,
          dataURL,
          wasmURL
        ).then((module) => new FaustCompiler_default(new LibFaust_default(module)));
      }
      return _FaustDspGenerator2.compilerPromise;
    };
    const args = "-ftz 2";
    try {
