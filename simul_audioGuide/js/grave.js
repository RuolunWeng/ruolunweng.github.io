...
function graveModule(global, foreign, buffer) {
	...
	function getNumInputsgraveSIG0(dsp) {
		dsp = dsp | 0;
		return 0;
	}
	
	function getNumOutputsgraveSIG0(dsp) {
		dsp = dsp | 0;
		return 1;
	}
	
	function instanceInitgraveSIG0(dsp, samplingFreq) {
		dsp = dsp | 0;
	...
	function fillgraveSIG0(dsp, count, output) {
		dsp = dsp | 0;
		count = count | 0;
		output = output | 0;
		var i = 0;
	...
	function classInit(dsp, samplingFreq) {
		dsp = dsp | 0;
		samplingFreq = samplingFreq | 0;
		instanceInitgraveSIG0(dsp, (samplingFreq | 0));
		fillgraveSIG0(dsp, 512, (dsp + 0 | 0));
...
function getSizegrave() {
	return 4737244;
}

function getPathTablegrave() {
	var pathTable = [];
...
faust.grave = function (context, buffer_size) {
    ...
    var jon_object = JSON.parse(getJSONgrave());
    ...
    var memory_size = pow2limit(getSizegrave() + (numIn + numOut) * (ptr_size + (buffer_size * sample_size)));
    ...
    // ASM module
    var factory = graveModule(window, window.Math, HEAP);
    console.log(factory);
 
    var pathTable = getPathTablegrave();
