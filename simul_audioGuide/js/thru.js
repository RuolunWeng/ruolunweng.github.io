...
function thruModule(global, foreign, buffer) {
	...
	function compute(dsp, count, inputs, outputs) {
		...
		fSlow0 = +(0.100000001 * +(max_f(0., +(+(HEAPF[dsp + 12 >> 2])))));
...
function getSizethru() {
	return 16;
}

function getPathTablethru() {
	var pathTable = [];
	pathTable["/0x00/izone"] = 12;
	return pathTable;
}

function getJSONthru() {
	return "{  \"name\": \"thru\",  \"inputs\": \"2\",  \"outputs\": \"2\",  \"meta\": [    { \"name\": \"thru\" },   { \"signals.lib/name\": \"Faust Signal Routing Library\" },   { \"signals.lib/version\": \"0.0\" }  ],  \"ui\": [    {    \"type\": \"vgroup\",    \"label\": \"0x00\",    \"items\": [      {      \"type\": \"vslider\",      \"label\": \"izone\",      \"address\": \"/0x00/izone\",      \"init\": \"0\",      \"min\": \"0\",      \"max\": \"1\",      \"step\": \"0.001\"     }    ]   }  ] } ";
}

function metadatathru(m) {
	m.declare("name", "thru");
...
faust.thru = function (context, buffer_size) {
    ...
    var jon_object = JSON.parse(getJSONthru());
    ...
    var memory_size = pow2limit(getSizethru() + (numIn + numOut) * (ptr_size + (buffer_size * sample_size)));
    ...
    // ASM module
    var factory = thruModule(window, null, HEAP);
    console.log(factory);

    var pathTable = getPathTablethru();
